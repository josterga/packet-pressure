...
![Multiple game turns showing terminal UI, tableau, and the player's hand](assets/terminal-gif.gif)
...
Only the exit node's packet value scores — there is no cumulative sum across the whole route. The PKT number printed on each card is its packet value — the points you score if it's your exit node. The player who holds the exit node collects those points.
